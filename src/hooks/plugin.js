...
export const usePluginStore = defineStore('pluginStore', () => {
    ...
    // Set background
    const setBackground = (img, device, clearIcon = true) => {
    ...
    class Actions {
        ...
        title;
        titleParameters = {};
        constructor(action, context, settings) {
            this.action = action;
        ...
        // Register screensaver event (tell software to take exclusive control of screensaver, can maintain screensaver entry time independently)
        registrationScreenSaverEvent = (device) => {
            server.send(JSON.stringify({
...
export const useWatchEvent = (type, MessageEvents) => {
    const plugin = usePluginStore();
    if (type === 'plugin') {
        return watch(() => plugin.message, () => {
            if (!plugin.message)
                return;
            if (plugin.message.action)
                return;
            ...
            if (plugin.message.event === 'didReceiveGlobalSettings') {
                plugin.globalSettings = plugin.message.payload.settings;
            }
    ...
    return watch(() => plugin.message, () => {
        if (!plugin.message)
            return;
        if (plugin.message.action !== MessageEvents['ActionID'])
            return;
